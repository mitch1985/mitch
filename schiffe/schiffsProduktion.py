...
def schiffsschleifeLoeschen(response):
    ...
    for produktionAttributeString in produktionsListe:
        ...
        responseLoescheSchiffe = requests.get(
            f'http://www.earthlost.de/produktion.phtml?action=delete&sid={sid}&item={produktionAttributeArray[0]}').text
...
for planetenIndex in meinePlaneten:
    ...
    responsePlanetProduktion = requests.get(
        f'http://www.earthlost.de/produktion.phtml?planetindex={planetenIndex}&sid={sid}').text
    ...
    responseSchiffeproduzieren = requests.post(f'http://www.earthlost.de/produktion.phtml?planetindex={planetenIndex}',
                                               data=schiffsproduktion).text
